...
public class Vista
{
	...
	private  String[] nombreColumnas = {"Nombre y apellido","Telefono", "Email", "Signo Zodiacal", "Fecha", "Contacto","Pais", "Prov", "Localidad", "Calle" , "Altura", "Depto","Piso","Cod Postal"};
	...
	{
		frame = new JFrame();
		frame.setBounds(200, 100, 1100, 300);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		
		JPanel panel = new JPanel();
		panel.setBounds(0, 0, 1100, 262);
		frame.getContentPane().add(panel);
		panel.setLayout(null);
		
		JScrollPane spPersonas = new JScrollPane();
		spPersonas.setBounds(10, 11, 1065, 182);
		...
		tablaPersonas.getColumnModel().getColumn(1).setPreferredWidth(230);
	...
	public void llenarTabla(List<PersonaDTO> personasEnTabla) {
		...
		for (PersonaDTO p : personasEnTabla)
		{
			...
			String signo = p.getSignoZodiacal().toString();

			String cp = p.getDomicilio().getCodPostal();

			String fecha = p.getFechaNac();
			String contacto = p.getTipoDeContacto();
			String pais = p.getDomicilio().getPais();
			String prov = p.getDomicilio().getProvincia();
			...
			Object[] fila = {nombre, tel, email, signo, fecha, contacto, pais, prov, localidad, calle , altura, depto,piso, cp};
